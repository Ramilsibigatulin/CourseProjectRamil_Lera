...
def add_task():
    ...
    db_cursor.execute(
        "INSERT INTO tasks (task_name, employee_name) VALUES (%s, %s)", (task_name, employee_name))
    db_connection.commit()
...
def generate_tasks():
    ...
    if not employees or not tasks:
        messagebox.showerror(
            "Ошибка", "Добавьте сотрудников и задачи перед генерацией")
        return
...
# root = Tk()
root.config(bg='#0047ab')  # Изменение фона на кобальт синий цвет
# или
root['bg'] = '#0047ab'  # Изменение фона на кобальт синий цвет
...
task_label = tk.Label(root, text="Название задачи:",
...
employee_label = tk.Label(root, text="Имя сотрудника:",
                          font=font_normal, bg=bg_color, fg=fg_color)
...
add_task_button = tk.Button(root, text="Добавить задачу",
                            command=add_task, font=font_normal, bg="#3b2e94", fg="white")
add_task_button.grid(row=2, column=0, columnspan=2, pady=10)
add_task_button.place(x=430, y=10)

generate_button = tk.Button(root, text="Генератор распределения задач",
                            command=generate_tasks, font=font_normal, bg="#1a153f", fg="white")
generate_button.grid(row=3, column=0, columnspan=2, pady=10)
generate_button.place(x=20, y=120)

show_tasks_button = tk.Button(root, text="Показать список задач из БД",
                              command=show_tasks, font=font_normal, bg="#1e2460", fg="white")
show_tasks_button.grid(row=4, column=0, columnspan=2, pady=10)
show_tasks_button.place(x=350, y=120)
